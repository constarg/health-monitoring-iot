...
health_monitor::movement_detection(void)
{
    ...
    bool is_moving_started_sig;
    bool is_freefall_sig;

    bool is_walking_continued_sig;
    bool is_jumping_continued_sig;
    bool is_falling_continued_sig;
    ...
    for (int sample = 0; sample < SENSORS_RECORD_DEPTH; sample++) {
        ...
        if (is_freefall_sig) {
            // TODO: detected falling continued.
            if (detected_jumping_continued_sig(&movement)) {
                is_jumping_continued_sig = true;
            }

        } else if (is_moving_started_sig) {
            if (detected_walking_continued_sig(&movement)) {
